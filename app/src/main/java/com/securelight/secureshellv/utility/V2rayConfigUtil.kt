...
import com.securelight.secureshellv.statics.EConfigType
import com.securelight.secureshellv.statics.ERoutingMode
...
//        val configType = configJson.getJSONArray("outbounds").getJSONObject(0).getString("protocol")
...
//    }
    ...
    private fun getV2rayNonCustomConfig(
        ...
        //取得默认配置
        if (TextUtils.isEmpty(v2rayJson)) {
            return result
        }

        //转成Json
        val v2rayConfig = Gson().fromJson(v2rayJson, V2rayConfig::class.java) ?: return result
...
//        dns(v2rayConfig)
        ...
        if (VpnSettings.localDnsEnabled) {
...
//        }
    ...
    private fun inbounds(v2rayConfig: V2rayConfig): Boolean {
        try {
            val socksPort = VpnSettings.socksPort
            val httpPort = VpnSettings.httpPort

            v2rayConfig.inbounds.forEach { curInbound ->
                if (!VpnSettings.proxySharing) {
                    //bind all inbounds to localhost if the user requests
                    curInbound.listen = "127.0.0.1"
                }
            }
            v2rayConfig.inbounds[0].port = socksPort
            val fakeDns = VpnSettings.fakeDnsEnabled
...
//                        domains = geositeCn.plus(proxyDomain).plus(directDomain)
            ...
            if (v2rayConfig.inbounds.none { e -> e.protocol == "dokodemo-door" && e.tag == "dns-in" }) {
                val dnsInboundSettings = V2rayConfig.InboundBean.InSettingsBean(
                    address = VpnSettings.dnsHost,
                    port = 53,
                    network = "tcp,udp"
                )

                val localDnsPort = VpnSettings.localDnsPort
